...
async function enterRoom(create) {
    ...
    if (!name || (!create && !roomId)) {
        $("connection-status").textContent = "Indiquez votre pseudo et le code de la partie.";
        return;
    }
    if (create) {
        ...
        const snapshot = await new Promise((done) => onValue(roomRef(), done, { onlyOnce: true }));
        if (!snapshot.exists()) {
            $("connection-status").textContent = "Salon introuvable.";
            return;
        }
        role = "player";
        await update(ref(db, `rooms/${roomId}/players/${clientId}`), { name, active: true });
...
function eliminate() {
    const totals = Object.values(state.votes || {}).reduce((out, id) => ({ ...out, [id]: (out[id] || 0) + 1 }), {});
    const target = Object.keys(totals).sort((left, right) => totals[right] - totals[left])[0];
    if (!target) return;
    const remaining = activePlayers(state).length - 1;
    change({ [`players/${target}/active`]: false, votes: {}, phase: remaining === 1 ? "ended" : "lobby", ...gameEvent("end") });
    if (remaining === 1) {
        const winner = activePlayers(state).find(([id]) => id !== target)?.[1];
        const key = winner.name.toLowerCase().replace(/[^a-z0-9]/gi, "-");
        runTransaction(ref(db, `seasonLeaderboard/${key}`), (entry) => ({ name: winner.name, score: (entry?.score || 0) + (state.bank || 0) }));
    }
}
...
export async function startGameApp() {
    ...
    $("room-id").addEventListener("input", () => { $("room-id").value = formatRoomCode($("room-id").value); });
    $("role-form").onsubmit = (submit) => { submit.preventDefault(); enterRoom(submit.submitter?.value === "create"); };
